...
function sendWhatsAppDirectOrder(existingOrder = null) {
  const brand = (window.BRANDS_CONFIG && window.BRANDS_CONFIG[currentBrand]) || window.BRANDS_CONFIG?.['Get pattas '] || { name: 'Get Pattas Kadai', themeColor: '#7c3aed' };
  const brandColor = brand.themeColor || '#7c3aed';
...
function handleWhatsAppOrderSubmit(e) {
  ...
  const shopCode = getShopShortCode ? getShopShortCode(currentBrand) : '002';
...
function getShopInvoiceTitle(brandKey) {
  return 'Get Pattas - Wholesale Shop-002';
}
