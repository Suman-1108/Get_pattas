...
function sendWhatsAppDirectOrder(existingOrder = null) {
  const brand = (window.BRANDS_CONFIG && window.BRANDS_CONFIG[currentBrand]) || window.BRANDS_CONFIG?.['muthu'] || { name: 'Get Pattas Kadai', themeColor: '#dc2626' };
  ...
  if (!cart || cart.length === 0) {
    ...
    const tableEl = document.getElementById('products') || document.getElementById('priceListContainer') || document.getElementById('brands');
...
function handleWhatsAppOrderSubmit(e) {
  ...
  const shopCode = getShopShortCode ? getShopShortCode(currentBrand) : '001';
  const shopTitle = getShopInvoiceTitle ? getShopInvoiceTitle(currentBrand) : (brand.shortName || brand.name || 'Get Pattas Kadai');
  ...
  if (cart.length === 0) {
    previewList.innerHTML = `
      <div class="co-empty-state" style="text-align: center; padding: 2rem 0.5rem; color: #64748b;">
        ...
        <button type="button" class="btn btn-primary" onclick="closeCheckoutModal(true); (document.getElementById('products') || document.getElementById('priceListContainer') || document.getElementById('brands'))?.scrollIntoView({behavior:'smooth'});" style="font-size: 0.82rem; padding: 0.4rem 0.9rem; border-radius: 6px;">
